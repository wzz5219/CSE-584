...
def process_csv(file_path, label, n_rows):
    ...
    with open(file_path, 'r') as file:
        content = file.read()
    #this is needed for csv understanding
    content = content.replace('"', '')
    cleaned_data = StringIO(content)
    df = pd.read_csv(cleaned_data, sep='|', on_bad_lines='skip')
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        input_ids[:len(inputs)] = torch.tensor([ord(c) for c in inputs], dtype=torch.long)
